...
Line 39 introduces a new local variable `tasks` into scope, which is at first
`nil`. We expect the call to `select` to be invoked on the return value of the
`tasks` getter, but since the local variable `tasks` is introduced first,
it is invoked on the local variable, which is initally `nil` (hence the error).

This demonstrates the importance of being careful when using local variables
with the same name as instance methods or accessor methods.

To fix this, we can simply rename the local variable to something that won't
interfere with the getter.
